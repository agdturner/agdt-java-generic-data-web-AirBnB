package uk.ac.leeds.ccg.web.airbnb;
...
public class Web_ScraperAirbnb extends Web_Scraper {
    ...
    public void init() throws IOException {
        ...
        dir = Paths.get(System.getProperty("user.dir"), "data", "airbnb",
                place, date);
...
//        place = "Sunderland";
    ...
    public void parsePlace(String url) throws IOException {
        ...
        if (!overwrite) {
            ...
            Path outputFile = Paths.get(dir.toString(), filename);
            if (Files.exists(outputFile)) {
                Data_ReadTXT reader = new Data_ReadTXT(env.de);
                lines = reader.read(outputFile, null, 1); // Not sure if 1 is right here, needs testing!
            } else {
                outputPW = getPrintWriter(homesurl + ".html");
                lines = getHTML(10, 1, homesurl, outputPW);
                outputPW.close();
        ...
        outputPW.close();
        Path resultDetail = Paths.get(dir.toString(), "result.dat");
